...
- [x] Карточка товара показывает средний рейтинг и количество отзывов.
...
- [x] README с командами запуска создан.
- [x] Server запускался для финальной HTTP-проверки.
- [x] Каталог, Swagger, регистрация и JWT-вход проверены через HTTP.
...
1. Подключить GitHub-авторизацию и выполнить push.
2. После backend можно начинать отдельный frontend-проект.
